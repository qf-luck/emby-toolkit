      ...
      <n-page-header>
        ...
        <template #footer>
          <n-alert title="操作提示" type="info" style="margin-top: 24px;">
           <li><b>配置规则:</b> 首先通过【默认订阅配置】或在每个演员的【订阅配置】标签页中，设定好您想要的筛选规则（如发行年份、题材、评分等）。</li>
           <li><b>自动处理:</b> 添加新演员或点击【手动刷新】后，系统会根据您的规则自动扫描演员的所有作品。</li>
           <li><b>查看结果:</b> 扫描完成后，点开演员卡片即可在不同的标签页中查看自动化处理的结果。所有不符合规则的作品都会被自动归类到【已忽略】列表，并附带忽略原因。</li>
          </n-alert>
        </template>
      </n-page-header>
